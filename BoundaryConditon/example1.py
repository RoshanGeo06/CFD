...
from PyFoam.ParaView.ParaViewPipeline import ParaviewPipeline
...
def create_simulation_environment(mesh_file, surface_vessel):
    ...
    # Create a ParaView pipeline for visualization
    pv = ParaviewPipeline(mesh)

    # Convert the mesh to VTK format
    pv.convertToVTK()

    # Run the ParaView utility to visualize the results
    runner = UtilityRunner(pv)
    runner.setup()
    runner.start()
    runner.join()
